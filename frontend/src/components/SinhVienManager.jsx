...
import { faEdit, faTrash, faSearch, faKey, faUsers, faSchool, faUserGraduate } from "@fortawesome/free-solid-svg-icons";
...
const SinhVienManager = ({ refreshKey }) => {
  ...
  const handlePhanLop = async () => {
    if (!selectedLop || selectedSinhVien.length === 0) {
      toast.error("Vui lòng chọn lớp và ít nhất một sinh viên.");
      return;
    }
    ...
    try {
      ...
      toast.success("Phân lớp thành công!");
      fetchSinhVienList();
      fetchSinhVienKhongLop();
    } catch (error) {
      toast.error(error.response?.data?.message || "Đã có lỗi xảy ra khi phân lớp.");
    }
  };
  ...
  const handleShowEditModal = (sinhVien) => {
    ...
    setFormData({
      ...
      maLop: sinhVien.lopHoc?.maLop || sinhVien.maLop || "",
      ...
      <Modal show={showPhanLopModal} onHide={() => setShowPhanLopModal(false)} backdrop="static" size="lg">
        ...
        <Modal.Body>
          <Form>
            ...
            <div className="mb-3">
              <div className="d-flex justify-content-between align-items-center mb-2">
                <Form.Label className="mb-0">Danh sách sinh viên chưa có lớp</Form.Label>
                <Button 
                  variant="link" 
                  size="sm" 
              ...
              <div style={{ maxHeight: '300px', overflowY: 'auto' }} className="border p-2 rounded">
                {sinhVienKhongLopList.length === 0 ? (
                  <p className="text-center mb-0">Không có sinh viên nào chưa được phân lớp</p>
                ) : (
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handlePhanLop}>
            Phân lớp
          </Button>
        </Modal.Footer>
      </Modal>
