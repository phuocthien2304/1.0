...
public class YeuCauMuonPhongController {
    ...
    public ResponseEntity<?> timPhongTrong(
            @RequestParam Date thoiGianMuon,
            @RequestParam Date thoiGianTra,
        ...
        try {
            ...
            if (loaiPhong != null && !loaiPhong.isEmpty()) {
                danhSachPhong = phongRepository.findByLoaiPhong(loaiPhong);
            } else {
            ...
            // Kiểm tra từng phòng xem có trùng lịch không
            danhSachPhong.removeIf(phong -> {
                List<YeuCauMuonPhong> trungLichPhong = yeuCauMuonPhongRepository.kiemTraTrungLichPhong(
                    phong.getMaPhong(),
                    thoiGianMuon,
                    thoiGianTra
