...
public class SinhVienController {
    ...
    public ResponseEntity<?> getLichSuMuonPhong() {
        ...
        // Thiết lập giá trị cho field coLichSuMuonPhong
        for (YeuCauMuonPhong yeuCau : lichSuMuon) {
            List<LichSuMuonPhong> lichSu = lichSuMuonPhongRepository.findByYeuCauMuonPhong(yeuCau);
            yeuCau.setCoLichSuMuonPhong(Boolean.valueOf(!lichSu.isEmpty()));
        }
    ...
    public ResponseEntity<?> getLichSuDaTra() {
        ...
        // Lọc chỉ giữ lại các yêu cầu đã có lịch sử mượn phòng
        List<YeuCauMuonPhong> yeuCauDaCoLichSu = yeuCauList.stream()
            .filter(yeuCau -> {
                Boolean coLichSu = yeuCau.getCoLichSuMuonPhong();
                return coLichSu != null && coLichSu;
            })
            .collect(Collectors.toList());
        
        return ResponseEntity.ok(yeuCauDaCoLichSu);
    }
