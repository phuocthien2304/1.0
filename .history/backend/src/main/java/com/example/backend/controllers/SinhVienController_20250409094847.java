...
public class SinhVienController {
    ...
    public ResponseEntity<?> baoSuCo(@RequestBody Map<String, Object> suCoData) {
        ...
        try {
            ...
            suCo.setTieuDe(tieuDe != null ? tieuDe : "Báo cáo sự cố phòng " + yeuCau.getPhong().getMaPhong());
            suCo.setMoTa(moTa);
            suCo.setThoiGianBao(new Date());
            
            if (mucDoStr != null && !mucDoStr.isEmpty()) {
                try {
                    suCo.setMucDo(SuCo.MucDo.valueOf(mucDoStr));
                } catch (IllegalArgumentException e) {
                    suCo.setMucDo(SuCo.MucDo.TRUNGBINH);
                }
            } else {
                suCo.setMucDo(SuCo.MucDo.TRUNGBINH);
            }
            ...
            // Lưu vào CSDL
            suCoService.luuBaoCaoSuCo(suCo);
