...
public class YeuCauMuonPhongController {
    ...
    @DeleteMapping("/sinhvien/huy/{maYeuCau}")
    @PreAuthorize("hasRole('SV')")
    public ResponseEntity<?> huyYeuCauSV(@PathVariable Integer maYeuCau) {
        try {
            // Kiểm tra người dùng hiện tại
            UserDetailsImpl userDetails = (UserDetailsImpl) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
            String userId = userDetails.getId();
            ...
            if (!yeuCau.getNguoiMuon().getIdNguoiDung().equals(userId)) {
    ...
    @DeleteMapping("/giangvien/huy/{maYeuCau}")
    @PreAuthorize("hasRole('GV')")
    public ResponseEntity<?> huyYeuCauGV(@PathVariable Integer maYeuCau) {
        try {
            // Kiểm tra người dùng hiện tại
            UserDetailsImpl userDetails = (UserDetailsImpl) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
            String userId = userDetails.getId();
