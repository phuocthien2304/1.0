...
public class SinhVienController {
    ...
    public ResponseEntity<?> baoSuCo(@RequestBody Map<String, Object> suCoData) {
        ...
        String mucDo = (String) suCoData.get("mucDo");
        ...
        try {
            ...
            SuCo suCo = new SuCo();
            suCo.setNguoiBaoCao(sinhVien);
            suCo.setPhong(yeuCau.getPhong());
            suCo.setTieuDe(tieuDe != null ? tieuDe : "Báo cáo sự cố phòng " + yeuCau.getPhong().getTenPhong());
            suCo.setMoTa(moTa);
            suCo.setThoiGianBao(new Date());
            
            // Xử lý mức độ sự cố
            if (mucDo != null && !mucDo.isEmpty()) {
                try {
                    suCo.setMucDo(SuCo.MucDo.valueOf(mucDo));
                } catch (IllegalArgumentException e) {
                    suCo.setMucDo(SuCo.MucDo.TRUNGBINH); // Giá trị mặc định
                }
            } else {
                suCo.setMucDo(SuCo.MucDo.TRUNGBINH); // Giá trị mặc định
            }
            ...
            suCo.setTrangThai(SuCo.TrangThai.DABAOCAO);
            
            if (maYeuCau != null) {
                suCo.setYeuCauMuonPhong(yeuCau);
            }
            
            if (maLichSu != null) {
                Optional<LichSuMuonPhong> lichSuOpt = lichSuMuonPhongRepository.findById(maLichSu);
                if (lichSuOpt.isPresent()) {
                    suCo.setLichSuMuonPhong(lichSuOpt.get());
                }
            }
            ...
            thongBaoService.guiThongBaoSuCo(suCo);
