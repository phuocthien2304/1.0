...
public class SinhVienController {
    ...
    @PostMapping("/yeucau-muonphong")
    @PreAuthorize("hasRole('SV')")
    public ResponseEntity<?> yeuCauMuonPhong(@RequestBody YeuCauMuonPhongRequest yeuCauMuonPhongRequest) {
        try {
            SinhVien sinhVien = getCurrentSinhVien();
            if (sinhVien == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
            ...
            // Kiểm tra số lượng yêu cầu mượn phòng hiện tại
            List<YeuCauMuonPhong> yeuCauHienTai = yeuCauMuonPhongRepository.findByNguoiGuiAndTrangThaiIn(
                sinhVien.getNguoiDung(),
                Arrays.asList(YeuCauMuonPhong.TrangThai.DADUYE, YeuCauMuonPhong.TrangThai.DANGXULY)
            );
            ...
            if (yeuCauMuonPhongService.kiemTraXungDotThoiGian(
                yeuCauMuonPhongRequest.getMaPhong(),
                yeuCauMuonPhongRequest.getThoiGianBatDau(),
                yeuCauMuonPhongRequest.getThoiGianKetThuc()
            )) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new MessageResponse("Thời gian mượn phòng bị xung đột với các yêu cầu khác"));
            }

            // Tạo yêu cầu mượn phòng
            YeuCauMuonPhong yeuCauMuonPhong = new YeuCauMuonPhong();
            yeuCauMuonPhong.setNguoiGui(sinhVien.getNguoiDung());
            yeuCauMuonPhong.setPhongHoc(phongHocRepository.findById(yeuCauMuonPhongRequest.getMaPhong())
                .orElseThrow(() -> new IllegalArgumentException("Không tìm thấy phòng học")));
            yeuCauMuonPhong.setThoiGianBatDau(yeuCauMuonPhongRequest.getThoiGianBatDau());
            yeuCauMuonPhong.setThoiGianKetThuc(yeuCauMuonPhongRequest.getThoiGianKetThuc());
            yeuCauMuonPhong.setLyDo(yeuCauMuonPhongRequest.getLyDo());
            yeuCauMuonPhong.setTrangThai(YeuCauMuonPhong.TrangThai.DANGXULY);
            yeuCauMuonPhong.setThoiGianTao(new Date());

            yeuCauMuonPhong = yeuCauMuonPhongRepository.save(yeuCauMuonPhong);

            // Tạo thông báo cho giảng viên
            thongBaoService.guiThongBao(
                sinhVien.getNguoiDung(),
                "Yêu cầu mượn phòng mới",
                "Sinh viên " + sinhVien.getNguoiDung().getHoTen() + " đã gửi yêu cầu mượn phòng " + yeuCauMuonPhong.getPhongHoc().getTenPhong(),
                null,
                null
            );
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new MessageResponse("Đã xảy ra lỗi khi gửi yêu cầu mượn phòng"));
        }
    }
