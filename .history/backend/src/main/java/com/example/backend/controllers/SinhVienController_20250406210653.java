...
public class SinhVienController {
    ...
    // 5.3 Cập nhật phản hồi về phòng (qua maYeuCau)
    @PutMapping("/phanhoi")
    @PreAuthorize("hasRole('SV')")
    public ResponseEntity<?> capNhatPhanHoi(@RequestBody PhanHoiRequest phanHoiRequest) {
        ...
        System.out.println("Yêu cầu cập nhật phản hồi SV: " + phanHoiRequest.getMaYeuCau() + ", đánh giá: " + phanHoiRequest.getDanhGia());
    ...
    public ResponseEntity<?> guiThongBao(@RequestBody ThongBaoRequest thongBaoRequest) {
        try {
            ...
            // Xử lý người nhận
            List<String> danhSachNguoiNhan = thongBaoRequest.getDanhSachNguoiNhan();
            if (danhSachNguoiNhan != null && !danhSachNguoiNhan.isEmpty()) {
                for (String idNguoiNhan : danhSachNguoiNhan) {
            ...
            // Xử lý gửi cho lớp (nếu có)
            if (thongBaoRequest.getGuiChoLop() && thongBaoRequest.getMaLop() != null) {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new MessageResponse("Lỗi khi gửi thông báo: " + e.getMessage()));
        }
    }
