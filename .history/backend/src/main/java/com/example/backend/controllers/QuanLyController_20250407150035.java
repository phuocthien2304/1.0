...
public class QuanLyController {
    ...
    public ResponseEntity<?> approveYeuCau(@PathVariable Integer maYeuCau) {
        ...
        ThongBaoGui thongBao = new ThongBaoGui();
        thongBao.setNguoiGui(quanLy.getNguoiDung());
        thongBao.setNguoiNhan(yeuCau.getNguoiMuon());
        thongBao.setTieuDe("Yêu cầu mượn phòng đã được duyệt");
        thongBao.setNoiDung("Yêu cầu mượn phòng " + yeuCau.getPhong().getMaPhong() + 
                           " từ " + formatDateTime(yeuCau.getThoiGianMuon()) + 
                           " đến " + formatDateTime(yeuCau.getThoiGianTra()) + 
                           " đã được duyệt.");
        thongBao.setDaDoc(false);
        thongBao.setThoiGian(new Date());
        
        thongBaoGuiRepository.save(thongBao);
