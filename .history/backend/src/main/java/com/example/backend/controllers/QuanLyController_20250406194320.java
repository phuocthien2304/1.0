    ...
    public ResponseEntity<?> getRoomFeedbackStatistics() {
        ...
        if (!allFeedbacks.isEmpty()) {
            averageRating = allFeedbacks.stream()
                .mapToDouble(PhanHoi::getDiemDanhGia)
                .average()
                .orElse(0);
        }
        ...
        List<Object[]> topRatedRoomsData = phanHoiRepository.findTopRatedRooms();
        List<Map<String, Object>> topRatedRooms = new ArrayList<>();
        
        // Lấy top 3 phòng có đánh giá cao nhất
        for (int i = 0; i < Math.min(3, topRatedRoomsData.size()); i++) {
            Object[] roomData = topRatedRoomsData.get(i);
            Phong phong = (Phong) roomData[0];
            Double rating = (Double) roomData[1];
            Long count = (Long) roomData[2];
        ...
        List<Object[]> lowRatedRoomsData = phanHoiRepository.findLowRatedRooms();
        List<Map<String, Object>> lowRatedRooms = new ArrayList<>();
        
        // Lấy top 3 phòng có đánh giá thấp nhất
        for (int i = 0; i < Math.min(3, lowRatedRoomsData.size()); i++) {
            Object[] roomData = lowRatedRoomsData.get(i);
            Phong phong = (Phong) roomData[0];
            Double rating = (Double) roomData[1];
            Long count = (Long) roomData[2];
        ...
        for (PhanHoi feedback : recentFeedbacksData) {
            ...
            feedbackInfo.put("maPhong", feedback.getPhong().getMaPhong());
            feedbackInfo.put("viTri", feedback.getPhong().getViTri());
            feedbackInfo.put("rating", feedback.getDiemDanhGia());
            feedbackInfo.put("comment", feedback.getNoiDung());
            feedbackInfo.put("userName", feedback.getNguoiDung().getHoTen());
        ...
        // Nếu chưa có dữ liệu thực, sử dụng dữ liệu mẫu
        if (allFeedbacks.isEmpty()) {
            // Thông tin tổng quát
