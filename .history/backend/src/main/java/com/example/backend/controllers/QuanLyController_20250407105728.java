...
public class QuanLyController {
    ...
    public ResponseEntity<?> getAllLopHoc() {
        try {
            // Giả sử có LopHocRepository, nếu chưa có thì cần tạo model và repository
            // List<LopHoc> lopHocList = lopHocRepository.findAll();
            
            // Tạm thời trả về danh sách giả lập
            List<Map<String, Object>> lopHocList = new ArrayList<>();
            
            // Tạo một số lớp mẫu
            Map<String, Object> lop1 = new HashMap<>();
            lop1.put("maLop", "CNTT01");
            lop1.put("tenLop", "Công nghệ thông tin 01");
            lop1.put("khoa", "Công nghệ thông tin");
            lop1.put("khoaHoc", "2020-2024");
            
            Map<String, Object> lop2 = new HashMap<>();
            lop2.put("maLop", "CNTT02");
            lop2.put("tenLop", "Công nghệ thông tin 02");
            lop2.put("khoa", "Công nghệ thông tin");
            lop2.put("khoaHoc", "2020-2024");
            
            Map<String, Object> lop3 = new HashMap<>();
            lop3.put("maLop", "KTPM01");
            lop3.put("tenLop", "Kỹ thuật phần mềm 01");
            lop3.put("khoa", "Công nghệ thông tin");
            lop3.put("khoaHoc", "2021-2025");
    ...
    // API lấy danh sách sinh viên chưa có lớp
    ...
    public ResponseEntity<?> getSinhVienKhongLop() {
        try {
            ...
            // Lọc sinh viên chưa có lớp (maLop là null hoặc rỗng)
            List<SinhVien> sinhVienKhongLop = new ArrayList<>();
            for (SinhVien sv : allSinhVien) {
                // Kiểm tra xem sinh viên có trường nào đại diện cho lớp không
                String maLop = null;
                
                // Thử các phương thức getter có thể có
                try {
                    // Thử gọi getMaLop nếu có
                    maLop = (String) sv.getClass().getMethod("getMaLop").invoke(sv);
                } catch (Exception e1) {
                    try {
                        // Thử gọi getLopHoc nếu có
                        maLop = (String) sv.getClass().getMethod("getLopHoc").invoke(sv);
                    } catch (Exception e2) {
                        // Không tìm thấy phương thức phù hợp, coi như không có lớp
                        maLop = null;
                    }
                }
                
                if (maLop == null || maLop.trim().isEmpty()) {
                    sinhVienKhongLop.add(sv);
                }
            }
            ...
            for (SinhVien sv : sinhVienKhongLop) {
                ...
                // Sử dụng reflection để lấy các giá trị
                try { svInfo.put("maSV", getProperty(sv, "getMaSV")); } catch (Exception e) {}
                try { svInfo.put("hoTen", getProperty(sv, "getHoTen")); } catch (Exception e) {}
                try { svInfo.put("email", getProperty(sv, "getEmail")); } catch (Exception e) {}
                try { svInfo.put("lienHe", getProperty(sv, "getLienHe")); } catch (Exception e) {}
                try { svInfo.put("gioiTinh", getProperty(sv, "getGioiTinh")); } catch (Exception e) {}
    ...
    // Phương thức helper để lấy giá trị thuộc tính bằng reflection
    private Object getProperty(Object obj, String getterMethodName) {
        try {
    ...
    public ResponseEntity<?> createLopHoc(@RequestBody Map<String, Object> request) {
        try {
            ...
            String khoa = (String) request.get("khoa");
            String khoaHoc = (String) request.get("khoaHoc");
            ...
            // Giả lập việc tạo lớp học thành công
            // Trong thực tế, cần tạo model LopHoc và repository tương ứng
            // LopHoc lopHoc = new LopHoc(maLop, tenLop, khoa, khoaHoc);
            // lopHocRepository.save(lopHoc);
            
            Map<String, Object> lopHocInfo = new HashMap<>();
            lopHocInfo.put("maLop", maLop);
            lopHocInfo.put("tenLop", tenLop);
            lopHocInfo.put("khoa", khoa);
            lopHocInfo.put("khoaHoc", khoaHoc);
            lopHocInfo.put("message", "Lớp học đã được tạo thành công");
            
            return ResponseEntity.status(HttpStatus.CREATED).body(lopHocInfo);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<?> phanLopSinhVien(@RequestBody Map<String, Object> request) {
        try {
            ...
            // Thử cập nhật sinh viên vào lớp
            int successCount = 0;
            for (String maSV : maSinhViens) {
                try {
                    ...
                    if (sinhVienOpt.isPresent()) {
                        ...
                        // Thử cập nhật maLop bằng các phương thức khác nhau
                        boolean updated = false;
                        
                        try {
                            // Thử tìm và gọi phương thức setMaLop
                            sinhVien.getClass().getMethod("setMaLop", String.class).invoke(sinhVien, maLop);
                            updated = true;
                        } catch (Exception e1) {
                            try {
                                // Thử tìm và gọi phương thức setLopHoc
                                sinhVien.getClass().getMethod("setLopHoc", String.class).invoke(sinhVien, maLop);
                                updated = true;
                            } catch (Exception e2) {
