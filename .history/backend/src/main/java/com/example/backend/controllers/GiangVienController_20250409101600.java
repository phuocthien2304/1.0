...
public class GiangVienController {
    ...
    public ResponseEntity<?> baoSuCoGV(@RequestBody Map<String, Object> suCoData) {
        ...
        // Get maLichSu from maYeuCau if available
        if (maYeuCau != null && (suCoData.get("maLichSu") == null || suCoData.get("maLichSu").toString().isEmpty())) {
            List<LichSuMuonPhong> lichSuList = lichSuMuonPhongRepository.findByYeuCauMuonPhongMaYeuCau(maYeuCau);
            if (!lichSuList.isEmpty()) {
                maLichSu = lichSuList.get(0).getMaLichSu();
            }
        }
        ...
        try {
            // Check if yeuCau is null
            if (yeuCau == null) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new MessageResponse("Không tìm thấy thông tin yêu cầu mượn phòng hợp lệ"));
            }
