...
class UserService {
  ...
  capNhatPhanHoi(phanHoiData) {
    console.log("Đang gọi API cập nhật phản hồi với dữ liệu:", JSON.stringify(phanHoiData));
    
    // Sử dụng endpoint đã tồn tại /phanhoi/{id} 
    // Nếu không có idPhanHoi, sử dụng maYeuCau để tìm phản hồi
    const idPhanHoi = phanHoiData.idPhanHoi || phanHoiData.maYeuCau;
    const API_ENDPOINT = `/sinhvien/phanhoi/${idPhanHoi}`;
    
    console.log(`Gọi API PUT đến: ${API_ENDPOINT} với data:`, phanHoiData);
    
    return api.put(API_ENDPOINT, phanHoiData)
      .then(response => {
        console.log("Kết quả cập nhật phản hồi:", response.data);
        return response;
      })
      .catch(error => {
