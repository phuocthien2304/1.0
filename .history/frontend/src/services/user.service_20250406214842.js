...
class UserService {
  ...
  async capNhatPhanHoi(phanHoiData) {
    try {
      console.log("Dữ liệu cập nhật phản hồi:", JSON.stringify(phanHoiData, null, 2));
      
      // Nếu có id hoặc idPhanHoi, sử dụng endpoint với ID
      if (phanHoiData.idPhanHoi) {
        console.log(`Gọi API cập nhật với ID: ${phanHoiData.idPhanHoi}`);
        const response = await api.put(`/sinhvien/phanhoi/${phanHoiData.idPhanHoi}`, phanHoiData);
        return response;
      } 
      // Nếu không có ID nhưng có mã yêu cầu, thử dùng endpoint không có ID
      else if (phanHoiData.maYeuCau) {
        console.log(`Gọi API cập nhật với mã yêu cầu: ${phanHoiData.maYeuCau}`);
