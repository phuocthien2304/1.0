...
class UserService {
  ...
  capNhatPhanHoi(phanHoiData) {
    console.log("Đang gọi API cập nhật phản hồi với dữ liệu:", phanHoiData);
    return api.put('/api/sinhvien/phanhoi', phanHoiData)
      .then(response => {
        console.log("Kết quả cập nhật phản hồi:", response.data);
        // Đảm bảo cập nhật bộ nhớ cache hoặc trạng thái của ứng dụng nếu cần
        return response;
      })
      .catch(error => {
