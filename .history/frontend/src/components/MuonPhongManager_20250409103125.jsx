...
const MuonPhongManager = ({ 
  ...
  const handleBaoSuCo = async (yeuCau) => {
    try {
      ...
      // Check if an incident has already been reported for this request
      const response = await UserService.kiemTraDaBaoSuCo(formattedYeuCau.maYeuCau);
      
      if (response.data.dabaocao) {
        // If an incident has already been reported, show an error message
        if (onError) onError("Bạn đã báo cáo sự cố cho yêu cầu này rồi.");
      } else {
        // If no incident has been reported, open the modal
        setSelectedYeuCau(formattedYeuCau);
        setSuCoForm({
      ...
      if (onError) onError(error.response?.data?.message || "Không thể kiểm tra trạng thái báo cáo sự cố.");
      setLoading(false);
    }
  };
