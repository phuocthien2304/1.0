...
const SuCoManager = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Xử lý dữ liệu thống kê từ backend
      const thongKeData = {
        ...
        thongKeTheoPhong: thongKeResponse.data || [] // Lấy trực tiếp dữ liệu từ response
      };
    ...
    <Container fluid>
      ...
      <Row className="mb-4">
        <Col md={8}>
          <Card>
            <Card.Body>
              <Card.Title>Thống kê sự cố theo phòng</Card.Title>
              <Table striped bordered hover size="sm">
                ...
                <tbody>
                  {thongKe.thongKeTheoPhong.map((item, index) => (
                    <tr key={index}>
                      <td>{item[0]?.maPhong}</td>
                      <td>{item[1]}</td>
                    </tr>
                  ))}
                  {thongKe.thongKeTheoPhong.length === 0 && (
                    <tr>
                      <td colSpan="2" className="text-center">Không có dữ liệu thống kê</td>
                    </tr>
                  )}
                </tbody>
              </Table>
