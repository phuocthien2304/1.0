...
const SinhVienBoard = ({ currentUser }) => {
  ...
  const refreshLichSuMuon = async () => {
    try {
      // Gọi API lấy yêu cầu mượn phòng
      const yeuCauRes = await UserService.getLichSuMuonPhong();
      setYeuCauMuonPhong(yeuCauRes.data);
      
      try {
        // Gọi API lấy lịch sử phòng đã trả trong khối try riêng biệt
        const lichSuDaTraRes = await UserService.getLichSuMuonPhongDaTraAPI();
        setLichSuMuonPhong(lichSuDaTraRes.data);
        console.log("Refreshed lichSuMuonPhong:", lichSuDaTraRes.data);
      } catch (historyError) {
        console.error("Error fetching room return history:", historyError);
        // Không hiển thị thông báo lỗi cho người dùng khi chỉ lịch sử trả phòng bị lỗi
      }
    } catch (error) {
      console.error("Error fetching room rental requests:", error);
      // Chỉ hiển thị lỗi nếu không thể lấy được yêu cầu mượn phòng
      // và khi người dùng đã mở tab mượn phòng (để tránh hiển thị lỗi khi đang ở tab khác)
      if (activeTab === "muonphong") {
        toast.error("Không thể cập nhật thông tin yêu cầu mượn phòng");
      }
    }
  };
