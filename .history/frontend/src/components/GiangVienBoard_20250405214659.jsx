...
const GiangVienBoard = () => {
  ...
  const fetchGiangVienInfo = async () => {
    try {
      ...
      // Since GiangVienController doesn't return avatarURL, we need to create a proper structure
      // for UserInfoDisplay to work with the avatar
      
      // If the data doesn't have a nguoiDung property, create one
      if (!gvData.nguoiDung) {
        gvData.nguoiDung = {
          hoTen: gvData.hoTen,
          email: gvData.email,
          ...
          avatarURL: "avatar5.jpg" // Using a different default avatar for teachers
        };
      } 
      // If there is a nguoiDung property but no avatarURL
      else if (!gvData.nguoiDung.avatarURL) {
        gvData.nguoiDung.avatarURL = "avatar5.jpg";
      }
      
      // If avatarURL is directly in the root object, but no nguoiDung
