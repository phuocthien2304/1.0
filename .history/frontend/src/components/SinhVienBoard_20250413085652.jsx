...
const SinhVienBoard = ({ currentUser }) => {
  ...
  const refreshLichSuMuon = async () => {
    // Chuyển sang tab mượn phòng
    setActiveTab("muonphong");
    
    try {
      // Tải lại dữ liệu yêu cầu mượn phòng
      try {
        const yeuCauRes = await UserService.getLichSuMuonPhong();
        setYeuCauMuonPhong(yeuCauRes.data);
        console.log("Refreshed yeuCauMuonPhong:", yeuCauRes.data);
      } catch (requestError) {
        console.error("Error fetching room rental requests:", requestError);
        // Không hiển thị toast lỗi tại đây
      }
      
      // Tải lại dữ liệu lịch sử mượn phòng
      try {
        const lichSuDaTraRes = await UserService.getLichSuMuonPhongDaTraAPI();
        setLichSuMuonPhong(lichSuDaTraRes.data);
      ...
      console.error("Error in refreshLichSuMuon:", generalError);
      // Chỉ hiển thị lỗi khi thực sự cần thiết
    }
  };
