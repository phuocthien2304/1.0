    ...
    try {
      ...
      if (response.data && typeof response.data === 'object') {
        ...
        if (response.data.allUsers && Array.isArray(response.data.allUsers)) {
          userList = [...userList, ...response.data.allUsers.map(user => ({
            idNguoiDung: user.id,
            hoTen: user.ten,
        ...
        userList = response.data.map(user => ({
          idNguoiDung: user.id || user.idNguoiDung,
          hoTen: user.ten || user.hoTen || user.tenNguoiDung,
          vaiTro: user.vaiTro || "Không xác định",
          maLop: user.maLop || user.lop || ""
        }));
      }
      console.log("Danh sách người nhận:", userList);
      // Loại bỏ người dùng hiện tại và chỉ lấy SV, GV
