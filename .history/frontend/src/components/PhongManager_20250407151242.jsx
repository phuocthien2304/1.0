...
const PhongManager = () => {
  ...
  const handleAddPhong = async () => {
    ...
    if (!formData.sucChua || isNaN(parseInt(formData.sucChua, 10))) {
      setMessage("Vui lòng nhập sức chứa là một số hợp lệ.");
      setMessageType("danger");
      return;
    }
  ...
  const handleChangePhongStatus = async (phong, newStatus) => {
    // Kiểm tra trạng thái hợp lệ - chỉ cho phép chuyển từ TRONG sang BAOTRI
    if (newStatus === "BAOTRI" && phong.trangThai !== "TRONG") {
      setMessage("Chỉ có thể đánh dấu bảo trì cho phòng đang ở trạng thái trống.");
