...
const MuonPhongManager = ({ 
  ...
  const handleBaoSuCo = (yeuCau) => {
    ...
    setSuCoForm({ 
      ...
      maLichSu: formattedYeuCau.maLichSu || null,
      moTa: "" 
    });
  ...
  const handleSuCoSubmit = async (e) => {
    ...
    try {
      ...
      // Create a copy of the form data and remove maLichSu if it's null
      const formData = { ...suCoForm };
      if (formData.maLichSu === null) {
        delete formData.maLichSu;
      }
      
      let response;
      if (isGiangVien) {
        response = await UserService.baoSuCoGV(formData);
      } else {
        response = await UserService.baoSuCo(formData);
      }
