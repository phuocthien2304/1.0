...
const GiangVienManager = ({ refreshKey }) => {
  ...
  const fetchGiangVienList = async () => {
    ...
    try {
      ...
      // Lấy danh sách khoa sau khi có dữ liệu giảng viên
      fetchKhoaList();
      setLoading(false);
    } catch (error) {
  ...
  const fetchKhoaList = async () => {
    try {
      // Lấy danh sách khoa từ dữ liệu giảng viên hiện có
      const uniqueKhoas = [...new Set(
        giangVienList
      ...
      const formattedKhoaList = uniqueKhoas.map(khoa => ({
        maKhoa: khoa,
        tenKhoa: khoa
      }));
      
      setKhoaList(formattedKhoaList);
    } catch (error) {
  ...
  // Lọc giảng viên chưa có khoa
  const giangVienKhongKhoa = giangVienList.filter(gv => !gv.khoa || gv.khoa.trim() === '');
          ...
          <Form>
            ...
            <div className="mb-3">
              <div className="d-flex justify-content-between align-items-center mb-2">
                <h6 className="mb-0">Danh sách giảng viên chưa phân khoa</h6>
                <Button 
                  variant="link" 
                  onClick={() => {
                    if (selectedGiangVien.length === giangVienKhongKhoa.length) {
                      setSelectedGiangVien([]);
                    } else {
                      setSelectedGiangVien(giangVienKhongKhoa.map(gv => gv.maGV));
                    }
                  }}
                  className="p-0"
                >
                  {selectedGiangVien.length === giangVienKhongKhoa.length ? "Bỏ chọn tất cả" : "Chọn tất cả"}
                </Button>
              </div>
              
              {giangVienKhongKhoa.length === 0 ? (
                <div className="text-center text-muted py-3">
                  Không có giảng viên nào chưa được phân khoa
                </div>
              ) : (
