...
const MuonPhongManager = ({ 
  ...
  const handleMuonPhongNangCaoSubmit = async (e) => {
    ...
    if (!muonPhongNangCaoForm.maPhong || !muonPhongNangCaoForm.soChoDat || !muonPhongNangCaoForm.thoiGianMuon || !muonPhongNangCaoForm.thoiGianTra || !muonPhongNangCaoForm.mucDich) {
    ...
    try {
      ...
      // Cập nhật tab requests và chuyển sang tab đó
      try {
        // Chuyển sang tab "requests" để người dùng có thể xem yêu cầu vừa tạo
        setActiveTab("requests");
        
        // Chờ một chút trước khi gọi onRefresh để tránh race condition
        setTimeout(() => {
          if (onRefresh) {
            // Gọi hàm refresh để cập nhật lại dữ liệu
            onRefresh();
          }
        }, 300);
      } catch (refreshError) {
        console.error("Lỗi khi cập nhật dữ liệu:", refreshError);
        // Không hiển thị lỗi refresh cho người dùng vì đã gửi yêu cầu thành công
      }
