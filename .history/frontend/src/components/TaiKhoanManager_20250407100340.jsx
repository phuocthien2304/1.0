...
import { faEdit, faTrash, faLock, faUnlock, faUserPlus, faSearch } from "@fortawesome/free-solid-svg-icons";
...
const TaiKhoanManager = () => {
  ...
  const handleAddTaiKhoan = async () => {
    ...
    formData.vaiTro = formData.vaiTro.replace("ROLE_", "");
  ...
  const handleShowEditModal = (taiKhoan) => {
    ...
    setFormData({
      ...
      maUser: taiKhoan.maSinhVien || taiKhoan.maGiangVien || taiKhoan.maQuanLy || "",
  ...
  const handleShowDeleteConfirm = (taiKhoan) => {
    setCurrentTaiKhoan(taiKhoan);
    setShowConfirmModal(true);
  };

  // Xóa tài khoản
  const handleDeleteTaiKhoan = async () => {
    try {
      await axios.delete(
        `${API_URL}/taikhoan/${currentTaiKhoan.id}`,
        { headers: authHeader() }
      ...
      setMessage(error.response?.data?.message || "Đã có lỗi xảy ra khi xóa tài khoản.");
      setMessageType("danger");
      setShowConfirmModal(false);
    }
  };
  ...
  return (
    <Container fluid>
                  ...
                  filteredTaiKhoan.map((taiKhoan) => (
                    <tr key={taiKhoan.id}>
                      ...
                      <td>
                        ...
                        <Button variant="outline-danger" size="sm" onClick={() => handleShowDeleteConfirm(taiKhoan)}>
        ...
        <Modal.Body>
          <Form>
            ...
            <Row>
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>Mã số <span className="text-danger">*</span></Form.Label>
                  <Form.Control
                    type="text"
                    name="maUser"
                    value={formData.maUser}
                    onChange={handleInputChange}
                    placeholder={formData.vaiTro === "ROLE_SV" ? "MSSV" : formData.vaiTro === "ROLE_GV" ? "Mã GV" : "Mã QL"}
                    required
                  />
                </Form.Group>
              </Col>
      ...
      <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)}>
        ...
        <Modal.Body>
          <p>Bạn có chắc chắn muốn xóa tài khoản <strong>{currentTaiKhoan?.hoTen}</strong>?</p>
        ...
        <Modal.Footer>
          ...
          <Button variant="danger" onClick={handleDeleteTaiKhoan}>
            Xóa tài khoản
          </Button>
        </Modal.Footer>
      </Modal>
