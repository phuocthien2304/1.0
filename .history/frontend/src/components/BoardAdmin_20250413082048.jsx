...
const BoardAdmin = () => {
  ...
  const [sortOrder, setSortOrder] = useState("desc"); // New state for sorting order (desc = giảm dần, asc = tăng dần)
  const [sortColumn, setSortColumn] = useState("treHan"); // New state to track which column is being sorted
  ...
  const fetchThongKeTraPhong = async () => {
    setLoading(true);
    try {
      const response = await UserService.getThongKeTraPhong();
  ...
  const handleSort = (column) => {
    ...
    if (sortColumn === column) {
      ...
      // If clicking a different column, set it as the sort column and default to desc
      setSortColumn(column);
      setSortOrder("desc");
    }
  };

  // Function to get the sort indicator for column headers
                        ...
                        {thongKeTraPhong ? (
                          <div>
                            <div className="mb-4">
                              ...
                              {/* Sắp xếp theo Trễ hạn */}
                              <Row className="mb-4">
                                <Col md={12}>
                                  <Card>
                                    <Card.Body>
                                      <h5 className="mb-3">Sắp xếp theo Trễ hạn</h5>
                                      <div className="d-flex align-items-center">
                                        <Form.Select 
                                      ...
                                      <div className="mt-3">
                                        <small className="text-muted">Bạn cũng có thể nhấp vào tiêu đề cột "Trễ hạn" trong bảng để sắp xếp dữ liệu.</small>
                                      </div>
                                    </Card.Body>
                                  </Card>
                                </Col>
                                      ...
                                      <Table striped bordered hover responsive>
                                        <thead>
                                          <tr>
                                            ...
                                            <th>Đúng hạn</th>
                                            <th 
                                              onClick={() => handleSort("treHan")} 
                                              style={{ cursor: "pointer" }}
                                              className="user-select-none"
                                            >
                                              Trễ hạn{getSortIndicator("treHan")}
                                            </th>
                                        ...
                                        <tbody>
                                          {thongKeTraPhong.thongKeTheoNguoiMuon
                                            .sort((a, b) => {
                                              ...
                                              if (sortColumn === "treHan") {
                                                return sortOrder === "desc" ? b.treHan - a.treHan : a.treHan - b.treHan;
                                              }
                                              return 0;
                                            })
                                            .map((item, index) => {
                                              ...
                                              return (
                                                <tr key={index}>
                                                  ...
                                                  <td>
                                                    <div className="d-flex align-items-center">
                                                      <div style={{ width: "60px" }}>{item.treHan}</div>
                                                      <ProgressBar 
                                                        style={{ height: "8px", flex: 1 }}
                                                        variant={item.treHan < 3 ? "danger" : "warning"} 
                                                        now={item.treHan} 
                                                      />
                                                    </div>
                                                  </td>
