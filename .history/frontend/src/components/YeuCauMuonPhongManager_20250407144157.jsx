...
import { Container, Row, Col, Card, Table, Button, Form, Modal, Alert, Badge, Tabs, Tab, ProgressBar } from "react-bootstrap";
...
const YeuCauMuonPhongManager = () => {
  ...
  const handleChoMuon = async (maYeuCau) => {
    try {
      ...
      setMessage("Đã ghi nhận việc cho mượn phòng thành công!");
      setMessageType("success");
      fetchYeuCauList();
      ...
      setMessage(error.response?.data?.message || "Đã có lỗi xảy ra khi cho mượn phòng.");
      setMessageType("danger");
    }
  };
        ...
        <Modal.Body>
          {selectedYeuCau && (
            <>
              ...
              {selectedYeuCau && selectedYeuCau.trangThai === "DADUYET" && (
                <div className="d-flex mt-4">
                  <Button 
                    variant="primary" 
