...
const BoardAdmin = () => {
  ...
  const handleReplyToFeedback = async (feedback) => {
    ...
    try {
      ...
      // Cập nhật UI (trong thực tế, bạn sẽ cần phải cập nhật dữ liệu roomFeedbackStats)
      alert("Đã phản hồi thành công!");
      setLoading(false);
    } catch (error) {
      console.error("Lỗi khi phản hồi đánh giá:", error);
      alert("Có lỗi xảy ra khi phản hồi đánh giá.");
      setLoading(false);
    }
  };
  ...
  const handleFlagFeedback = async (feedback) => {
    ...
    try {
      ...
      alert("Có lỗi xảy ra khi đánh dấu phản hồi.");
      setLoading(false);
    }
  };
  ...
  const handleTaiKhoanAdded = (vaiTro) => {
    ...
    // Nếu là tài khoản sinh viên và đang ở tab quản lý tài khoản, chuyển sang tab quản lý sinh viên
              ...
              {getFilteredFeedbacks().map((feedback, index) => (
                <div className="list-group-item" key={index}>
                  ...
                  <div className="d-flex justify-content-between">
                    <small className="text-muted">Người phản hồi: {feedback.userName}</small>
                    <div>
                      <Button variant="outline-secondary" size="sm" className="me-1" onClick={() => handleReplyToFeedback(feedback)}>
                        <FontAwesomeIcon icon={faReply} /> Phản hồi
                      </Button>
                      <Button variant="outline-primary" size="sm" onClick={() => handleFlagFeedback(feedback)}>
                        <FontAwesomeIcon icon={faFlag} /> Đánh dấu
                      </Button>
                    </div>
                  </div>
                </div>
