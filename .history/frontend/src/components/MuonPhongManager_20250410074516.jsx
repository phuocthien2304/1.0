    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (phongMockData.length === 0) {
          // Giả lập gợi ý phòng gần nhất
          const goiY = [];
          
          // Tạo gợi ý sớm hơn 2 giờ
          const thoiGianMuonSom = new Date(muonPhongNangCaoForm.thoiGianMuon);
          thoiGianMuonSom.setHours(thoiGianMuonSom.getHours() - 2);
          
          const thoiGianTraSom = new Date(thoiGianMuonSom);
          const duration = muonPhongNangCaoForm.thoiGianTra - muonPhongNangCaoForm.thoiGianMuon;
          thoiGianTraSom.setTime(thoiGianTraSom.getTime() + duration);
          
          // Tạo gợi ý muộn hơn 2 giờ
          const thoiGianMuonMuon = new Date(muonPhongNangCaoForm.thoiGianMuon);
          thoiGianMuonMuon.setHours(thoiGianMuonMuon.getHours() + 2);
          
          const thoiGianTraMuon = new Date(thoiGianMuonMuon);
          thoiGianTraMuon.setTime(thoiGianTraMuon.getTime() + duration);
          
          // Lấy ngẫu nhiên 2 phòng từ danh sách phòng thỏa mãn điều kiện
          ...
          if (availableRooms.length > 0) {
            if (thoiGianMuonSom.getHours() >= 7) {
              goiY.push({
                phong: availableRooms[0],
                thoiGianMuon: thoiGianMuonSom,
                thoiGianTra: thoiGianTraSom
              });
            }
            
            if (thoiGianTraMuon.getHours() < 22) {
              goiY.push({
                phong: availableRooms.length > 1 ? availableRooms[1] : availableRooms[0],
                thoiGianMuon: thoiGianMuonMuon,
                thoiGianTra: thoiGianTraMuon
              });
            }
          }
          
          setGoiYPhongGanNhat(goiY);
        } else {
          setGoiYPhongGanNhat([]);
        }
