...
const MuonPhongManager = ({ 
  ...
  const loaiPhongOptions = ["Phòng học", "Phòng hội thảo", "Phòng máy tính", "Phòng thí nghiệm", "Phòng họp"];
  ...
  const handleTimPhongTrong = async () => {
    try {
      ...
      // Giả lập API call để tìm phòng trống
      // Trong thực tế, bạn sẽ gọi API từ UserService
      setTimeout(() => {
        // Giả lập kết quả tìm phòng
        const phongMockData = danhSachPhong.filter(phong => {
          // Lọc theo số chỗ ngồi
          if (parseInt(phong.sucChua) < parseInt(muonPhongNangCaoForm.soChoDat)) {
            return false;
          }
          
          // Lọc theo loại phòng (nếu có)
          if (muonPhongNangCaoForm.loaiPhong && phong.loaiPhong !== muonPhongNangCaoForm.loaiPhong) {
            return false;
          }
          
          // Giả lập kiểm tra phòng trống theo thời gian
          // Trong thực tế, logic này sẽ được thực hiện ở backend
          const isAvailable = Math.random() > 0.7; // 30% phòng sẽ được coi là trống
          
          return isAvailable;
        });
        
        setPhongTrongList(phongMockData);
        
        // Nếu không có phòng trống, tạo gợi ý
        if (phongMockData.length === 0) {
          // Lấy thời gian người dùng yêu cầu
          const userRequestStart = new Date(muonPhongNangCaoForm.thoiGianMuon);
          const userRequestEnd = new Date(muonPhongNangCaoForm.thoiGianTra);
          const requestDuration = userRequestEnd - userRequestStart;
          
          // Tạo các khung giờ trống có thể cho ngày hiện tại
          const currentDate = new Date();
          let timeSlots = [];
          
          // Lấy ngẫu nhiên 2-4 phòng từ danh sách phòng thỏa mãn điều kiện
          const availableRooms = danhSachPhong.filter(phong => 
            parseInt(phong.sucChua) >= parseInt(muonPhongNangCaoForm.soChoDat) && 
            (!muonPhongNangCaoForm.loaiPhong || phong.loaiPhong === muonPhongNangCaoForm.loaiPhong)
          );
          
          if (availableRooms.length > 0) {
            // Tạo khung giờ từ 7h đến 22h, mỗi khung 2 giờ
            const startDate = new Date(userRequestStart);
            startDate.setHours(7, 0, 0, 0);
            
            const endDay = new Date(userRequestStart);
            endDay.setHours(22, 0, 0, 0);
            
            // Tạo danh sách các khung giờ 2 giờ trong ngày
            while (startDate.getTime() < endDay.getTime()) {
              const slotStart = new Date(startDate);
              const slotEnd = new Date(startDate);
              slotEnd.setTime(slotStart.getTime() + requestDuration); // Sử dụng cùng thời lượng như người dùng yêu cầu
              
              // Chỉ thêm khung giờ nếu khung kết thúc trước 22h và sau thời gian hiện tại
              if (slotEnd.getHours() < 22 && slotStart > currentDate) {
                const randomRoom = availableRooms[Math.floor(Math.random() * availableRooms.length)];
                timeSlots.push({
                  phong: randomRoom,
                  thoiGianMuon: slotStart,
                  thoiGianTra: slotEnd
                });
              }
              
              // Tăng lên 1 giờ cho mỗi lần lặp
              startDate.setHours(startDate.getHours() + 1);
            }
            
            // Tính khoảng cách thời gian từ khung giờ này đến yêu cầu của người dùng
            timeSlots.forEach(slot => {
              const slotMidpoint = new Date(slot.thoiGianMuon.getTime() + (slot.thoiGianTra - slot.thoiGianMuon) / 2);
              const userMidpoint = new Date(userRequestStart.getTime() + requestDuration / 2);
              
              // Tính khoảng cách bằng phút giữa trung điểm của hai khung giờ
              slot.distance = Math.abs(slotMidpoint - userMidpoint) / 60000; // Chuyển đổi từ milliseconds sang phút
            });
            
            // Sắp xếp theo khoảng cách tăng dần (gần nhất đầu tiên)
            timeSlots.sort((a, b) => a.distance - b.distance);
            
            // Chỉ giữ lại tối đa 3 gợi ý gần nhất
            timeSlots = timeSlots.slice(0, 3);
          }
          
          setGoiYPhongGanNhat(timeSlots);
        } else {
          setGoiYPhongGanNhat([]);
        }
      ...
      // Trong thực tế, bạn sẽ gọi API như sau:
      // const response = await UserService.timPhongTrong({
      //   thoiGianMuon: muonPhongNangCaoForm.thoiGianMuon.toISOString(),
      //   thoiGianTra: muonPhongNangCaoForm.thoiGianTra.toISOString(),
      //   soChoDat: muonPhongNangCaoForm.soChoDat,
      //   loaiPhong: muonPhongNangCaoForm.loaiPhong
      // });
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <DatePicker
                    selected={muonPhongNangCaoForm.thoiGianMuon}
                    onChange={(date) => setMuonPhongNangCaoForm(prev => ({ ...prev, thoiGianMuon: date, thoiGianTra: null }))}
                    showTimeSelect
                    timeFormat="HH:mm"
                    timeIntervals={timeIntervals}
                    filterTime={filterTime}
                    ...
                    {goiYPhongGanNhat.map((goiY, index) => (
                      <Card key={index} className="mb-2">
                        <Card.Body>
                          ...
                          <Card.Text>
                            Sức chứa: {goiY.phong.sucChua} người<br />
                            Thời gian trống: {formatDate(goiY.thoiGianMuon)} - {formatDate(goiY.thoiGianTra)}<br />
                            <small className="text-muted">
                              {Math.floor(goiY.distance / 60) > 0 ? `${Math.floor(goiY.distance / 60)} giờ ` : ''}
                              {Math.floor(goiY.distance % 60)} phút {goiY.thoiGianMuon > muonPhongNangCaoForm.thoiGianMuon ? 'sau' : 'trước'} thời gian yêu cầu
                            </small>
                          </Card.Text>
                          <Button 
                            variant="outline-primary" 
