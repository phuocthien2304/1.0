...
const SinhVienBoard = ({ currentUser }) => {
  ...
  const refreshLichSuMuon = async () => {
    console.log("Refreshing room rental data...");
    
    // Hiển thị trạng thái loading
    setLoading(true);
    
    try {
      // Lấy dữ liệu mới từ máy chủ - thực hiện tuần tự để đảm bảo chúng được cập nhật đúng
      // Lấy yêu cầu mượn phòng
      const yeuCauRes = await UserService.getYeuCauMuonPhong();
      setYeuCauMuonPhong(yeuCauRes.data);
      console.log("Updated room requests data:", yeuCauRes.data);
      
      // Lấy lịch sử mượn phòng
      const lichSuDaTraRes = await UserService.getLichSuMuonPhongDaTraAPI();
      setLichSuMuonPhong(lichSuDaTraRes.data);
      console.log("Updated room history data:", lichSuDaTraRes.data);
      
      // Hiển thị thông báo thành công
      // toast.success("Đã cập nhật dữ liệu mới nhất");
    } catch (error) {
      console.error("Error refreshing room data:", error);
      // Chỉ hiển thị lỗi nếu người dùng đang ở tab mượn phòng
