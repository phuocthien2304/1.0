...
const GiangVienBoard = () => {
  ...
  const fetchGiangVienInfo = async () => {
    try {
      ...
      // Đảm bảo thông tin được định dạng chính xác
      if (!gvData.nguoiDung) {
        // Nếu không có đối tượng nguoiDung, tạo một đối tượng mới
        gvData.nguoiDung = {
          hoTen: gvData.hoTen,
          email: gvData.email,
          lienHe: gvData.lienHe,
          gioiTinh: gvData.gioiTinh,
          avatarURL: gvData.avatarURL || "default-avatar.jpg"
        };
      } else if (!gvData.nguoiDung.avatarURL && gvData.avatarURL) {
        // Nếu có avatarURL ở cấp root nhưng không có trong nguoiDung
        gvData.nguoiDung.avatarURL = gvData.avatarURL;
      }
      
      // Đảm bảo có avatar mặc định nếu không có avatar
      if (!gvData.nguoiDung.avatarURL) {
        gvData.nguoiDung.avatarURL = "avatar5.jpg";
      }
      
      console.log("Processed giangVienInfo:", gvData); // Debug log
