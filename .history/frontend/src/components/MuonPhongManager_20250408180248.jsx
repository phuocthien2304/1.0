...
const MuonPhongManager = ({ 
  ...
  const handleHuyYeuCau = async () => {
    try {
      ...
      let response;
      if (isGiangVien) {
        response = await UserService.huyYeuCauMuonPhongGV(selectedYeuCau.maYeuCau);
      } else {
        response = await UserService.huyYeuCauMuonPhong(selectedYeuCau.maYeuCau);
      }
                          ...
                          <tr key={yc.maYeuCau}>
                            ...
                            <td>
                              <div className="d-flex gap-1">
                                ...
                                {yc.trangThai === 'DANGXULY' && new Date(yc.thoiGianMuon) > new Date() && (
                                  <Button
