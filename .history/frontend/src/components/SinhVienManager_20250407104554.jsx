...
const SinhVienManager = ({ refreshKey }) => {
  ...
  useEffect(() => {
    fetchSinhVienList();
    // Chỉ gọi các API không bắt buộc sau khi đã lấy được danh sách sinh viên chính
    // Cách này giúp tránh hiển thị nhiều lỗi khi không có quyền truy cập
  ...
  const fetchSinhVienList = async () => {
    setLoading(true);
    try {
      // Nếu không có xác thực, vẫn cố gắng lấy dữ liệu nhưng không hiển thị lỗi
      const user = JSON.parse(localStorage.getItem('user'));
      ...
      // Chỉ hiển thị lỗi nếu không phải là lỗi xác thực 401
  ...
  const fetchSinhVienKhongLop = async () => {
    try {
      ...
      const user = JSON.parse(localStorage.getItem('user'));
      if (!user || !user.token) {
        console.log("Không có token xác thực, bỏ qua việc lấy danh sách sinh viên không lớp");
        return;
      }
      
      // Kiểm tra xem API endpoint này có tồn tại không
      try {
        const response = await axios.get(`${API_URL}/sinhvien/khonglop`, { headers: authHeader() });
        setSinhVienKhongLopList(response.data);
      } catch (error) {
        if (error.response && error.response.status === 404) {
          // API endpoint không tồn tại, thử lấy từ API sinh viên thông thường và lọc
          console.log("API sinhvien/khonglop không tồn tại, sử dụng phương pháp thay thế");
          const allSinhVien = await axios.get(`${API_URL}/sinhvien`, { headers: authHeader() });
          const sinhVienKhongLop = allSinhVien.data.filter(sv => !sv.maLop || sv.maLop.trim() === '');
          setSinhVienKhongLopList(sinhVienKhongLop);
        } else {
      ...
      // Không hiển thị thông báo lỗi này, vì không ảnh hưởng đến chức năng chính
    }
  };
  ...
  const handlePhanLop = async () => {
    ...
    try {
      ...
      try {
        ...
        // Nếu API không tồn tại, thử phương pháp cập nhật từng sinh viên
        if (error.response && error.response.status === 404) {
          console.log("API phân lớp không tồn tại, sử dụng phương pháp thay thế");
          
          // Cập nhật từng sinh viên một
          let errorCount = 0;
          for (const maSV of selectedSinhVien) {
            try {
              await axios.put(
                `${API_URL}/sinhvien/${maSV}`,
                { maLop: selectedLop },
                { headers: authHeader() }
              );
            } catch (err) {
              console.error(`Lỗi khi cập nhật sinh viên ${maSV}:`, err);
              errorCount++;
            }
          }
          ...
          if (errorCount === 0) {
            ...
            setMessage(`Phân lớp thành công ${selectedSinhVien.length - errorCount}/${selectedSinhVien.length} sinh viên.`);
        ...
        <Card.Header>
          <div className="d-flex justify-content-between align-items-center mb-2">
            ...
            <div>
              ...
              {/* Chỉ hiển thị các nút quản lý lớp học khi user đã đăng nhập */}
                ...
                <>
                  ...
                  {sinhVienKhongLopList.length > 0 && (
                    <Button variant="warning" onClick={handleShowPhanLopModal}>
                      ...
                      <Badge bg="danger" className="ms-1">{sinhVienKhongLopList.length}</Badge>
                    </Button>
                  )}
                </>
              )}
        ...
        <Card.Body>
          {!JSON.parse(localStorage.getItem('user'))?.token && (
            <Alert variant="info" className="mb-3">
              Đăng nhập để sử dụng đầy đủ chức năng quản lý sinh viên và lớp học.
            </Alert>
          )}
